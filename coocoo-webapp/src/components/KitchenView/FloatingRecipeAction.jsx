...
const FloatingRecipeAction = ({ selectedCount, currentStyle, onGenerate, isTaskActive = false }) => {
  return (
    <div className={`fixed bottom-20 left-4 right-4 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-[600px] bg-slate-blue/95 backdrop-blur-md text-white p-md rounded-2xl shadow-2xl z-40 flex justify-between items-center gap-md border ${isTaskActive ? 'border-amber-400 ring-4 ring-amber-400/30' : 'border-ochre-gold/30'}`}>
      <div className="flex items-center gap-sm">
        <span className="material-symbols-outlined text-ochre-gold text-2xl animate-pulse">auto_awesome</span>
      ...
      <button
        ...
        className={`text-white font-extrabold px-lg py-2.5 rounded-xl text-xs shadow-md transition-all active:scale-95 whitespace-nowrap flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed ${
          isTaskActive 
            ...
            : 'bg-gradient-to-r from-[#E07A5F] to-primary hover:brightness-110 shadow-lg'
        }`}
      >
