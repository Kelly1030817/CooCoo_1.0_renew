...
describe("HTTP contract adapter", () => {
  ...
  test("returns the current state envelope", async () => {
    ...
    const body = (await response.json()) as {
      data: { missions: Array<{ key: string }>; fridgeProfile?: unknown };
    };
    expect(body).toHaveProperty("data");
    expect(body.data).not.toHaveProperty("fridgeProfile");
    expect(body.data.missions.map((mission) => mission.key)).toEqual([
      "cook_today",
      "eat_prepared",
      "use_expiring",
    ]);
  ...
  test("exposes deterministic recipe errors", async () => {
    const response = await app.handle(
      new Request("http://localhost/api/v1/recipes/generate", {
        method: "POST",
    ...
    expect(response.status).toBe(422);
    expect((await response.json()) as { error: { code: string; requestId: string } }).toMatchObject(
      { error: { code: "VALIDATION_ERROR" } },
    );
  });
  test("updates reminder categories while keeping the fixed weekly cap", async () => {
    const response = await app.handle(
      new Request("http://localhost/api/v1/settings/reminders", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    expect(response.status).toBe(200);
    expect(
      (await response.json()) as { data: { weeklyLimit: number; expiringIngredients: boolean } },
    ).toMatchObject({ data: { weeklyLimit: 3, expiringIngredients: false } });
  });
  test("applies a confirmed recipe adjustment to the MealTask and rejects an unknown preview", async () => {
    ...
    const previewResponse = await app.handle(
      new Request(`http://localhost/api/v1/recipes/${recipeId}/adjustments/preview`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    const preview = (await previewResponse.json()) as {
      data: { adjustedRecipe: { servings: number }; changes: unknown[] };
    };
    expect(preview.data.adjustedRecipe.servings).toBe(2);
    expect(preview.data.changes.length).toBeGreaterThan(0);

    const taskId = crypto.randomUUID();
    const taskResponse = await app.handle(
      new Request("http://localhost/api/v1/meal-tasks", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          operationId: taskId,
    ...
    expect(
      (await taskResponse.json()) as { data: { id: string; recipe: { servings: number } } },
    ).toMatchObject({ data: { id: taskId, recipe: { servings: 2 } } });
    ...
    const invalidResponse = await app.handle(
      new Request("http://localhost/api/v1/meal-tasks", {
        ...
        body: JSON.stringify({
          ...
          recipePackageId: recipeId,
          adjustmentPreviewId: crypto.randomUUID(),
    ...
    expect((await invalidResponse.json()) as { error: { code: string } }).toMatchObject({
      error: { code: "ADJUSTMENT_PREVIEW_INVALID" },
    });
  });
  test("keeps only the latest ten chef chats and deletes a selected session", async () => {
    for (let index = 0; index < 11; index += 1) {
      const response = await app.handle(
        new Request("http://localhost/api/v1/chef-chat/sessions", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            operationId: crypto.randomUUID(),
            message: `第 ${index + 1} 次相談`,
          }),
        }),
      );
      expect(response.status).toBe(200);
    ...
    const deleteResponse = await app.handle(
      new Request(`http://localhost/api/v1/chef-chat/sessions/${target.id}`, { method: "DELETE" }),
    );
    expect(deleteResponse.status).toBe(200);
    const after = (await (
      await app.handle(new Request("http://localhost/api/v1/chef-chat/sessions"))
...
describe("SQLite integration-test harness", () => {
  ...
  test("rolls back a failed transaction", () => {
    ...
    expect(
      sqlite.db
        .query<{ count: number }, []>("select count(*) as count from transaction_probe")
        .get()?.count,
    ).toBe(1);
  });
});
