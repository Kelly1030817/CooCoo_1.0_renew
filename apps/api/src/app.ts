...
export const app = new Elysia({ name: "coocoo-api" })
  ...
  .use(planningRoutes({authenticate:cloudDataEnabled?authenticateRequest:async()=>({id:"preview"}),context:cloudDataEnabled?catalogContext:previewPlanningContext,catalog:cloudDataEnabled?catalogRepository:undefined,repository:mealPlanRepository,onPlanSaved:cloudDataEnabled?(userId,plan,draft)=>shoppingRepository.syncWeeklyPlan(userId,plan.id,draft):(_userId,plan,draft)=>{const state=service.state();state.mealPlan=plan;state.shoppingItems=state.shoppingItems.filter(item=>item.mealPlanId!==plan.id||item.checked);repository.write(state);for(const item of draft)if(!state.shoppingItems.some(existing=>existing.mealPlanId===plan.id&&existing.name===item.name&&existing.unit===item.unit&&existing.checked))service.saveShopping({name:item.name,category:item.category,qty:item.quantity,unit:item.unit,checked:false,status:"本週一次備齊",estCost:0,mealPlanId:plan.id,source:"plan"})},reserveGenerate:cloudDataEnabled?async(userId,operationId,input)=>aiUsageRepository.reserve<RecipeGeneration>(userId,operationId,"recipe_generation",await aiUsageRepository.hash(input),process.env.OPENROUTER_MODEL||"google/gemini-3.7-flash",Number(process.env.RECIPE_AI_MAX_CALL_TWD||2)):undefined,settleGenerate:cloudDataEnabled?async(userId,operationId,status,actual,result)=>aiUsageRepository.settle(userId,operationId,status,actual,result):undefined}))
