import type { MealTaskRestockCommand, MealTaskRestockResult, ShoppingItem } from "@coocoo/contracts";
import { shoppingCategoryFor } from "@coocoo/core";
import { getSupabaseAdmin } from "../../shared/infrastructure/supabase";

const map=(row:Record<string,unknown>):ShoppingItem=>({id:String(row.id),name:String(row.name),category:row.category as ShoppingItem["category"],qty:Number(row.quantity),unit:String(row.unit),checked:Boolean(row.checked),status:String(row.status),estCost:Number(row.estimated_cost),shortageId:row.shortage_id?String(row.shortage_id):undefined,source:row.source?(row.source as ShoppingItem["source"]):undefined});
export class SupabaseShoppingRepository {
  ...
  async save(userId:string,item:Partial<ShoppingItem>&Pick<ShoppingItem,"name">){
    ...
    const values={user_id:userId,name:item.name,ingredient_key:canonicalKey,category:item.category||shoppingCategoryFor(item.name),quantity:item.qty??1,unit:item.unit||"包",checked:item.checked??false,status:item.status||"needed",estimated_cost:item.estCost??0,shortage_id:item.shortageId??null,source:item.source??null};
