...
export class CatalogRepository {
  ...
  async admin(user:string):Promise<CatalogAdminState>{await this.owner(user);const month=new Date(Date.now()+8*3600000).toISOString().slice(0,7);const since=month+'-01T00:00:00+08:00';
    const [versions,prices,control,usage,jobs,reports]=await Promise.all([this.versions(),this.prices(),this.db.from('recipe_catalog_control').select('*').single(),this.db.from('recipe_catalog_usage').select('*').gte('created_at',since),this.db.from('recipe_catalog_jobs').select('id,status,attempts,error,created_at,version_id').gte('created_at',since).order('created_at',{ascending:false}).limit(1000),this.db.from('recipe_catalog_reports').select('id,version_id,safety,message,created_at,processed_at,recipe_catalog_versions(recipe)').order('created_at',{ascending:false}).limit(100)]);
    ...
    const catalogBudgetTwd=Number(control.data.catalog_monthly_budget_twd??50),candidateLimit=Number(control.data.catalog_monthly_candidate_limit??50);
    const jobRows=jobs.data||[],failedJobCount=jobRows.filter(row=>row.status==='failed').length;
    const {stalePriceCount,expiringPriceCount}=referencePriceStatus(prices);
    return {versions,prices,paused:control.data.paused,month,spentTwd,reservedTwd,catalogBudgetTwd,candidateCount:jobRows.length,candidateLimit,failedJobCount,stalePriceCount,expiringPriceCount,lastRunAt:control.data.last_run_at,alerts:catalogAdminAlerts({spentTwd,reservedTwd,budgetTwd:catalogBudgetTwd,lastRunAt:control.data.last_run_at,failedJobCount,stalePriceCount,expiringPriceCount}),jobs:jobRows.map(row=>({id:row.id,status:row.status,attempts:row.attempts,error:row.error,createdAt:row.created_at,versionId:row.version_id})),reports:(reports.data||[]).map(row=>{const joined=row.recipe_catalog_versions as unknown as {recipe:{title?:string}}|{recipe:{title?:string}}[]|null;const relation=Array.isArray(joined)?joined[0]:joined;return{id:row.id,versionId:row.version_id,title:relation?.recipe?.title||'未知食譜',safety:row.safety,message:row.message,createdAt:row.created_at,processedAt:row.processed_at};})};
  }
