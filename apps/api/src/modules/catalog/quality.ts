...
export function inspectRecipe(value: unknown, existing: RecipePackage[]): CatalogReview {
  ...
  if(recipe.steps.some((s,i)=>s.order!==i+1||!s.instruction.trim()||!s.compactInstruction?.trim()||!s.guidance?.successCue.trim()||!s.voiceText.trim()||(s.timerSeconds!==null&&s.timerSeconds<0)))reasons.push('STEP_INVALID');
