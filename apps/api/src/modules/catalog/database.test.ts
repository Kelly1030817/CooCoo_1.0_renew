...
test('restock v2: partial keeps task open, replay is idempotent, expiry and location are honored',async()=>{
 const recipeId=crypto.randomUUID(),taskId=crypto.randomUUID(),taskOperation=crypto.randomUUID(),shoppingId=crypto.randomUUID(),shortageId=taskOperation+':蛋';
 await db.query("insert into public.recipes(id,user_id,title,servings,prep_minutes,total_minutes,cookware_types,ingredients,steps,safety_reviewed,source) values($1,$2,'MealTask flow',1,5,10,'{}','[{\"ingredientKey\":\"蛋\",\"name\":\"雞蛋\",\"quantity\":2,\"unit\":\"顆\",\"isPantryStaple\":false}]','[]' ,true,'brand_safe')",[recipeId,user]);
 await db.query("insert into public.meal_tasks(id,user_id,operation_id,recipe_id,status,current_meal,next_meal,planned_total_servings,shortages) values($1,$2,$3,$4,'needs_shopping',$5,$6,1,$7)",[taskId,user,taskOperation,recipeId,JSON.stringify({date:'2026-09-11',slot:'dinner',servings:1}),JSON.stringify({strategy:'skip'}),JSON.stringify([{id:shortageId,ingredientKey:'蛋',name:'雞蛋',quantity:2,unit:'顆',resolution:'needed'}])]);
 await db.query("insert into public.shopping_items(id,user_id,name,ingredient_key,quantity,unit,category,estimated_cost,checked,shortage_id,source) values($1,$2,'雞蛋','蛋',1,'顆','protein',15,true,$3,'task')",[shoppingId,user,shortageId]);
 const op1=crypto.randomUUID();
 const first=await db.query<{value:{count:number;mealTaskStatus:string;replayed:boolean;remainingShortages:Array<{quantity:number;resolution:string}>}}>("select public.restock_checked_shopping_v2($1,$2,$3,$4,1) value",[user,op1,JSON.stringify([{shopping_item_id:shoppingId,shortage_id:shortageId,actual_quantity:1,actual_unit:'顆',actual_price:15,storage_location:'cold',expires_on:'2026-09-20'}]),taskId]);
 expect(first.rows[0].value).toMatchObject({count:1,mealTaskStatus:'needs_shopping',replayed:false,remainingShortages:[{quantity:1,resolution:'needed'}]});
 const batch=await db.query<{expires_on:Date|string;location:string;unit_cost:string}>('select expires_on,location,unit_cost from public.inventory_batches where user_id=$1 and name=\'雞蛋\'',[user]);
 const expiresOnly=(value:Date|string)=>(value instanceof Date?value.toISOString():String(value)).slice(0,10);
 ...
 const replay=await db.query<{value:{count:number;replayed:boolean}}>("select public.restock_checked_shopping_v2($1,$2,$3,$4,1) value",[user,op1,JSON.stringify([{shopping_item_id:shoppingId,shortage_id:shortageId,actual_quantity:1,actual_unit:'顆',actual_price:15,storage_location:'cold',expires_on:'2026-09-20'}]),taskId]);
 expect(replay.rows[0].value).toMatchObject({count:1,replayed:true});
 expect((await db.query('select * from public.inventory_batches where user_id=$1 and name=\'雞蛋\'',[user])).rows).toHaveLength(1);
 await expect(db.query("select public.restock_checked_shopping_v2($1,$2,$3,$4,99)",[user,crypto.randomUUID(),'[]',taskId])).rejects.toThrow('MEAL_TASK_REVISION_CONFLICT');
 const shoppingId2=crypto.randomUUID();
 await db.query("insert into public.shopping_items(id,user_id,name,ingredient_key,quantity,unit,category,estimated_cost,checked,shortage_id,source) values($1,$2,'雞蛋','蛋',1,'顆','protein',15,true,$3,'task')",[shoppingId2,user,shortageId]);
 const op2=crypto.randomUUID();
 const second=await db.query<{value:{mealTaskStatus:string;nextActions:string[]}}>("select public.restock_checked_shopping_v2($1,$2,$3,$4,2) value",[user,op2,JSON.stringify([{shopping_item_id:shoppingId2,shortage_id:shortageId,actual_quantity:1,actual_unit:'顆',actual_price:15,storage_location:'pantry',expires_on:'2026-09-25'}]),taskId]);
 ...
 expect((await db.query<{location:string}>("select location from public.inventory_batches where user_id=$1 and name='雞蛋' order by created_at desc limit 1",[user])).rows[0].location).toBe('pantry');
