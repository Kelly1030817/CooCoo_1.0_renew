...
export function catalogAdminAlerts(input: {
  ...
  if (input.budgetTwd > 0 && input.spentTwd + input.reservedTwd >= input.budgetTwd * 0.8) {
    alerts.push(
      `食譜 AI 額度已達 80%（NT$${(input.spentTwd + input.reservedTwd).toFixed(2)} / NT$${input.budgetTwd.toFixed(2)}）`,
    );
  }
  ...
  if (input.failedJobCount > 0)
    alerts.push(`${input.failedJobCount} 個食譜工作失敗，請查看錯誤並決定是否重審`);
  if (input.stalePriceCount > 0)
    alerts.push(`${input.stalePriceCount} 筆參考價格已過期或日期異常，暫時不能判定符合預算`);
  if (input.expiringPriceCount > 0)
    alerts.push(`${input.expiringPriceCount} 筆參考價格將在 7 天內到期`);
  return alerts;
}
