...
export function reviewedSeedRecipes() {
  ...
  return brandSafeRecipes.map((source, index) => {
    ...
    if (!review.pass)
      throw new Error(`SEED_RECIPE_REJECTED:${recipe.title}:${review.reasons.join(",")}`);
    accepted.push(recipe);
    return {
      id: seedIds[index],
      eventId: eventIds[index],
      familyId: source.recipeId,
      recipe,
...
export async function ensureSeedCatalog(repo: CatalogRepository) {
  for (const seed of reviewedSeedRecipes()) {
    const version = await repo.db.from("recipe_catalog_versions").upsert(
      {
        id: seed.id,
    ...
    const reviews: Array<{
      version_id: string;
      reviewer: string;
      result: CatalogReview & Record<string, unknown>;
    }> = ["rules", "quality", "safety"].map((reviewer) => ({
      version_id: seed.id,
      reviewer,
      result: {
        ...seed.review,
        reviewMode: "curated_seed",
        ...(reviewer === "safety" ? { source: SAFETY_SOURCE } : {}),
      },
    }));
    const saved = await repo.db
      .from("recipe_catalog_reviews")
      .upsert(reviews, { onConflict: "version_id,reviewer", ignoreDuplicates: true });
    if (saved.error) throw saved.error;
    const event = await repo.db.from("recipe_catalog_events").upsert(
      {
        id: seed.eventId,
        version_id: seed.id,
        kind: "curated_seed_published",
  ...
  const retired = await repo.db
    .from("recipe_catalog_versions")
    .update({ status: "quarantined", reasons: ["初始食譜用量單位已由可查價版本取代"] })
    .in("id", retiredSeedIds)
    .eq("status", "published");
