...
export function syncRoutes() {
  return new Elysia({ name: "offline-sync" })
    .post(
      "/api/v1/sync",
      async ({ headers, body }) => {
        ...
        for (const operation of body.operations) {
          if (operation.id !== operation.payload.completionKey)
            throw new Error("OPERATION_ID_MISMATCH");
          const r = await new SupabaseCookingRepository().complete(user.id, operation.payload);
          results.push({
            id: operation.id,
            status: r.hasConflict ? "conflict" : "synced",
            ...(r.hasConflict ? { message: "料理結果已保存，庫存用量有衝突，請確認。" } : {}),
          });
        }
        return { data: { results } };
      },
    ...
    .get("/api/v1/sync/conflicts", async ({ headers }) => {
      const u = await authenticateRequest(headers.authorization);
    ...
    .post(
      "/api/v1/sync/conflicts/:id/acknowledge",
