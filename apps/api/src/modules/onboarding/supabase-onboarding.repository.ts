...
export class SupabaseOnboardingRepository {
  async save(userId: string, profile: OnboardingProfile) {
    const { error } = await getSupabaseAdmin().rpc("save_onboarding_profile", {
      p_user_id: userId,
      p_profile: profile,
    });
  ...
  async read(userId: string) {
    ...
    for (const result of [profile, cookware, restrictions, weeklyGoal, reminders])
      if (result.error) throw result.error;
    return {
      profile: profile.data,
      cookware: cookware.data,
