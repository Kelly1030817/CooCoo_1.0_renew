...
export class SupabaseReceiptRepository {
  async create(userId: string, image: File) {
    ...
    const { data, error } = await client
      .from("receipts")
      .insert({ id: receiptId, user_id: userId, original_image_path: path, status: "uploaded" })
      .select()
      .single();
    if (error) {
      await client.storage.from("receipt-images").remove([path]);
      throw error;
    }
    return data;
  }
  ...
  async image(userId: string, receiptId: string) {
    ...
    const { data, error: downloadError } = await client.storage
      .from("receipt-images")
      .download(receipt.original_image_path);
    if (downloadError) throw downloadError;
    return {
      receipt,
      bytes: new Uint8Array(await data.arrayBuffer()),
      mimeType: data.type as "image/jpeg" | "image/png" | "image/webp",
    };
  }

  async saveRecognition(userId: string, receiptId: string, recognition: ReceiptRecognition) {
    const client = getSupabaseAdmin();
    const rows = recognition.items.map((item) => ({
      ...item,
    ...
    const sanitized = rows.map(
      ({ unitPrice: _unitPrice, actualPrice: _actualPrice, ...row }) => row,
    );
    ...
    const { data, error } = await client
      .from("receipts")
      .update({
        purchased_on: recognition.purchasedOn,
        status: "needs_review",
  ...
  async markFailed(userId: string, receiptId: string, errorMessage: string) {
    await getSupabaseAdmin()
      ...
      .update({
        status: "failed",
        recognition_error: errorMessage,
        updated_at: new Date().toISOString(),
      })
      .eq("id", receiptId)
      .eq("user_id", userId);
  ...
  async confirm(userId: string, receiptId: string, items: unknown[]) {
    const { error } = await getSupabaseAdmin().rpc("confirm_receipt_and_restock", {
      p_user_id: userId,
  ...
  async delete(userId: string, receiptId: string) {
    ...
    const { error: storageError } = await client.storage
      .from("receipt-images")
      .remove([data.original_image_path]);
