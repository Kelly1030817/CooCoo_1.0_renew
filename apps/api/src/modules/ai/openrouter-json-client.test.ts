...
describe("OpenRouter structured client", () => {
    ...
    const fetcher = (async (_url: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          usage: { cost: 0.01, prompt_tokens: 10, completion_tokens: 2 },
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    }) as typeof fetch;
    const result = await new OpenRouterJsonClient("secret", "test-model", fetcher).generate<{
      ok: boolean;
    }>({
      system: "system",
      prompt: "prompt",
  ...
  test("marks provider HTTP failures as known non-billable responses", async () => {
    ...
    const promise = new OpenRouterJsonClient("secret", "test-model", fetcher).generate({
      system: "system",
      prompt: "prompt",
      schema: Type.Object({ ok: Type.Boolean() }),
      schemaName: "test",
