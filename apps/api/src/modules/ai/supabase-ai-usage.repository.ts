...
export class SupabaseAiUsageRepository {
  ...
  async reserve<T = unknown>(
    ...
    const { data, error } = await getSupabaseAdmin().rpc("reserve_ai_operation", {
      p_user: userId,
    ...
    if (reservation?.status === "failed" && reservation.result == null)
      throw new Error("AI_OPERATION_FAILED");
    return reservation?.status === "completed" || reservation?.status === "failed"
      ? (reservation.result as T)
      : null;
  }
  async settle(
    userId: string,
    ...
    const { error } = await getSupabaseAdmin().rpc("settle_ai_operation", {
      p_user: userId,
      p_operation: operationId,
      p_status: status,
      p_actual: actualTwd,
