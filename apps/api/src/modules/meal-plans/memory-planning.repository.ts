...
export class MemoryPlanningRepository implements PlanningRepository {
  ...
  async save(userId:string,plan:MealPlan){const key=userId+":"+plan.weekStart;if(!this.plans.has(key)){const packages=plan.meals.map(packageForMeal);this.plans.set(key,{plan:structuredClone(plan),packages});for(const recipe of packages)this.recipes.set(userId+":"+recipe.recipeId,recipe)}return (await this.current(userId,plan.weekStart))!}
  ...
  async savePackage(userId:string,recipe:RecipePackage){const id=crypto.randomUUID();const value={...recipe,id:"package-"+id,recipeId:id};this.recipes.set(userId+":"+id,structuredClone(value));return value}
}
