...
type RecipeRow={catalog_version_id?:string;source?:RecipePackage['source'];id:string;title:string;servings:number;prep_minutes:number;total_minutes:number;estimated_cost:number;cookware_types:string[];ingredients:RecipePackage["ingredients"];steps:RecipePackage["steps"];image_path:string|null;fallback_image_url:string};
...
export class SupabaseMealPlanRepository {
  ...
  async package(userId:string,recipeId:string){
    ...
    return {catalogVersionId:row.catalog_version_id,source:row.source,id:`package-${row.id}`,recipeId:row.id,title:row.title,servings:row.servings,prepMinutes:row.prep_minutes,totalMinutes:row.total_minutes,estimatedCost:row.estimated_cost,cookwareTypes:row.cookware_types,ingredients:row.ingredients,steps:row.steps,imageUrl:row.image_path,fallbackImageUrl:row.fallback_image_url,downloadedAt:null} satisfies RecipePackage;
  }
}
