...
import { assertDate, createMealPlan, createTodayDecision, refreshAvailability, rescheduleMeal, scalePackage, taipeiDate, weekOf, type MealPlanningContext } from "./meal-planning";
...
export interface PlanningRepository {
  ...
  save(userId:string,plan:MealPlan):Promise<{plan:MealPlan;packages:RecipePackage[]}>;
  ...
  savePackage(userId:string,recipe:RecipePackage,source:"gemini"|"brand_safe"):Promise<RecipePackage>;
}
export interface PlanningDependencies {
  ...
  afterGenerate?:(userId:string,source:"gemini"|"brand_safe"|"failed")=>Promise<void>;
}
export function planningRoutes(deps:PlanningDependencies){
  const withAvailability=(saved:{plan:MealPlan;packages:RecipePackage[]},context:MealPlanningContext)=>({...saved,...refreshAvailability(saved.plan,context.inventory)});
  return new Elysia({name:"planning-routes"})
    ...
    .post("/api/v1/meal-plans",async({headers,body})=>{
      ...
      const saved=existing||await deps.repository.save(user.id,createMealPlan(context));
    ...
    .get("/api/v1/recipes/:id/package",async({headers,params})=>{
      ...
      const brand=brandSafeRecipes.find(recipe=>recipe.recipeId===params.id);
    ...
    .post("/api/v1/recipes/generate",async({headers,body})=>{
      ...
      try {
        result=await(deps.generate||generateRecipe)({style:body.style||"台式家常",excludeTitle:body.excludeTitle,ingredientNames:body.ingredientIds.map(id=>context.ingredientIds[id]),restrictions:context.restrictions,cookware:context.cookware,budget:context.perMealBudget,energyLevel:"normal",inventory:context.inventory});
