...
export function weekOf(date: string) {
  const day = new Date(assertDate(date) + "T12:00:00Z").getUTCDay() || 7;
  return dateAt(date, 1 - day);
}
  ...
  return ingredients.map((ingredient) => {
    ...
    for (const stock of remaining) {
      if (
        ...
        ![stock.ingredientKey, stock.name || ""].some((key) =>
          [ingredient.ingredientKey, ingredient.name].map(normalize).includes(normalize(key)),
        )
      )
        continue;
...
export function scalePackage(
  ...
  return {
    ...
    servings,
    estimatedCost: Math.ceil(recipe.estimatedCost * factor),
    ingredients: coverIngredients(
      recipe.ingredients.map((item) => ({ ...item, quantity: item.quantity * factor })),
      inventory,
    ),
    downloadedAt: null,
  };
...
function eligiblePackages(context: MealPlanningContext) {
  if (context.strictCatalog) return context.recipes || [];
  return rankRecipes(
    context.recipes ?? brandSafeRecipes,
    {
      restrictions: context.restrictions,
      cookwareTypes: context.cookwareTypes,
      dailyBudget: context.perMealBudget,
      energyLevel: context.energyLevel || "normal",
    },
    context.inventory,
  ).map(({ recipe }) => scalePackage(recipe, context.servings, context.inventory));
}
export function createTodayDecision(
  context: MealPlanningContext,
  input: { date: string; slot: MealSlot },
): TodayDecision {
  ...
  return {
    ...
    alternatives: recipes.slice(1, 3),
    source: context.strictCatalog ? "catalog" : "brand_safe",
    notice: recipes.length
      ? `依你的飲食限制、廚具、時間與庫存挑選${context.strictCatalog ? "已發布" : "品牌"}食譜。`
      : "目前沒有符合飲食限制、廚具與庫存的餐點，請調整設定後再試。",
  };
}
export function createMealPlan(
  ...
  if (
    !context.mealSlots.length ||
    context.weeklyTarget < 1 ||
    context.weeklyTarget > context.mealSlots.length * 7
  )
    throw new Error("WEEKLY_TARGET_EXCEEDS_SLOTS");
  const id = options.id || (() => crypto.randomUUID());
  const slots = Array.from({ length: 7 }, (_, day) =>
    context.mealSlots.map((slot) => ({ date: dateAt(context.weekStart, day), slot })),
  ).flat();
  const meals: PlannedMeal[] = slots
    .slice(
      0,
      context.strictCatalog ? Math.min(context.weeklyTarget, recipes.length) : context.weeklyTarget,
    )
    .map((target, index) => {
      ...
      return {
        ...
        cookwareTypes: recipe.cookwareTypes,
        energyLevel: context.energyLevel || "normal",
      };
    });
...
export function unfilledMealSlots(context: MealPlanningContext, plan: MealPlan) {
  ...
  return Array.from({ length: 7 }, (_, day) =>
    ...
    .flat()
    .filter((slot) => !active.has(`${slot.date}:${slot.slot}`))
    .slice(0, Math.max(0, context.weeklyTarget - active.size));
}
export function refreshAvailability(plan: MealPlan, inventory: Stock[], today = taipeiDate()) {
  const active = plan.meals
    .filter((meal) => meal.status !== "cancelled" && meal.status !== "cooked")
    .sort(
      (a, b) =>
  ...
  const covered = coverIngredients(
    active.flatMap((meal) => meal.ingredients),
    inventory,
  );
  let offset = 0;
  const mapped = new Map(
    active.map((meal) => {
      const ingredients = covered.slice(offset, offset + meal.ingredients.length);
      offset += meal.ingredients.length;
      return [meal.id, { ...meal, ingredients }] as const;
    }),
  );
  const meals = plan.meals.map((meal) => mapped.get(meal.id) || meal);
  const expiryWarnings = Array.from(
    new Set(
      active.flatMap((meal) =>
        meal.ingredients.flatMap((ingredient) =>
          inventory
            .filter(
              (stock) =>
                normalize(stock.ingredientKey) === normalize(ingredient.ingredientKey) &&
                dateAt(today, stock.daysLeft) < meal.date,
            )
            .map(
              () =>
                ingredient.name +
  ...
  return {
    plan: {
      ...refreshPlanRates({ ...plan, meals }, plan.updatedAt),
      inventoryCoverageRate: requirements.length
        ? requirements.filter((i) => i.coveredByInventory).length / requirements.length
        : 0,
    },
    expiryWarnings,
  };
}
export function rescheduleMeal(
  plan: MealPlan,
  mealId: string,
  command: MealPostpone,
  ...
  let date = command.date,
    slot = command.slot;
  const occupied = (d: string, s: MealSlot) =>
    plan.meals.some(
      (item) =>
        item.id !== mealId && item.status !== "cancelled" && item.date === d && item.slot === s,
    );
  if (command.kind === "next_slot") {
    const order = ["breakfast", "lunch", "dinner"];
    const candidates = Array.from({ length: 8 }, (_, i) =>
      slots.map((s) => ({ date: dateAt(meal.date, i), slot: s })),
    )
      .flat()
      .filter((c) => c.date > meal.date || order.indexOf(c.slot) > order.indexOf(meal.slot));
    const target = candidates.find((c) => !occupied(c.date, c.slot));
    date = target?.date;
    slot = target?.slot;
  }
    ...
    meals: plan.meals.map((item) =>
      item.id !== mealId
        ...
        : {
            ...
            date: date || item.date,
            slot: slot || item.slot,
            status: command.kind === "cancel" ? ("cancelled" as const) : ("planned" as const),
          },
    ),
  };
}
export function packageForMeal(meal: PlannedMeal, recipes = brandSafeRecipes): RecipePackage {
  const source = recipes.find((recipe) => recipe.title === meal.title);
  if (!source) throw new Error("RECIPE_PACKAGE_NOT_FOUND");
  return {
    ...structuredClone(source),
    id: "package-" + meal.recipeId,
