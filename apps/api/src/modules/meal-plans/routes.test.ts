...
describe("meal planning HTTP routes", () => {
  ...
  test("creates a weekly plan once and returns the same persisted plan on retry", async () => {
    const request = () =>
      app.handle(
        new Request("http://localhost/api/v1/meal-plans", {
          method: "POST",
    ...
    const second = (await (await request()).json()) as {
      data: { plan: { id: string; meals: unknown[] } };
    };
    ...
    const body = (await response.json()) as {
      data: {
        ...
        recipe: {
          ...
          steps: Array<{ timerSeconds: number | null }>;
        };
      };
    };
    expect(body.data.source).toBe("brand_safe");
    expect(body.data.recipe.cookwareTypes).toContain("電磁爐");
    expect(body.data.recipe.ingredients).toContainEqual(
      expect.objectContaining({ quantity: 120, unit: "克" }),
    );
