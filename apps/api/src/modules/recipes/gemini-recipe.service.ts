...
export interface RecipeRequestContext{style:string;excludeTitle?:string;ingredientNames:string[];restrictions:DietaryRestriction[];cookware:Array<{type:string;capacity:string|null;limitations:string[]}>;budget:number;energyLevel:"low"|"normal";inventory:Array<{ingredientKey:string;daysLeft:number}>}
...
export async function generateRecipe(context:RecipeRequestContext):Promise<RecipeGeneration> {
  ...
  const fallback=rankRecipes(brandSafeRecipes,{restrictions:context.restrictions,cookwareTypes,dailyBudget:context.budget,energyLevel:context.energyLevel},context.inventory).find(item=>item.recipe.title!==context.excludeTitle)?.recipe;
  ...
  if(!fallback)throw new Error("NO_SAFE_RECIPE_AVAILABLE");return {recipe:structuredClone(fallback),source:"brand_safe",notice:"目前提供符合設定的品牌食譜；AI 食譜暫時無法使用。"};
}
