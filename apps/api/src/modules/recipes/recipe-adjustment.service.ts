...
function rulesAdjustment(recipe: RecipePackage, request: RecipeAdjustmentRequest) {
  ...
  const replacements = new Map(
    request.replacementRequests.map((item) => [
      normalize(item.ingredientKey),
      item.requestedReplacement.trim(),
    ]),
  );
  return {
    ...
    ingredients: recipe.ingredients.map((item) => {
      ...
      return {
        ...item,
        ...(replacement ? { ingredientKey: replacement, name: replacement } : {}),
...
export async function previewRecipeAdjustment(
  recipe: RecipePackage,
  request: RecipeAdjustmentRequest,
  inventory: InventoryItem[],
  restrictions: DietaryRestriction[],
  client?: OpenRouterJsonClient,
): Promise<RecipeAdjustmentPreview & { costUsd?: number }> {
  ...
  if (client) {
    try {
      const result = await client.generate<RecipePackage>({
        ...
        system:
          "你是 CooCoo 食譜調整器。只能依要求調整份數、替代食材與料理情境；不可放寬過敏、禁食或食安，不可新增使用者沒有的廚具。維持同一份權威料理包，instruction、compactInstruction 與 guidance 必須描述相同的食材用量、步驟、時間、溫度及完成標準，精簡指引不得刪除安全關鍵資訊。只輸出繁體中文 JSON。",
        prompt: JSON.stringify({
          original: recipe,
          servings: request.servings,
          replacements: request.replacementRequests,
          context: request.context,
          hardRestrictions: restrictions.filter((item) => item.isHardLimit),
        }),
      });
      if (!Value.Check(RecipePackageSchema, result.value)) throw new Error("AI_SCHEMA_INVALID");
      if (!hasCompleteRecipeGuidance(result.value))
        throw new Error("AI_RECIPE_GUIDANCE_INCOMPLETE");
      adjustedRecipe = {
        ...result.value,
        id: `${recipe.id}:adjusted:${request.operationId}`,
  ...
  if (request.replacementRequests.length && source !== "openrouter")
    throw new Error("REPLACEMENT_PREVIEW_UNAVAILABLE");
  ...
  const missing = adjustedRecipe.ingredients
      ...
      ? [
          {
            ...
            after: `${request.servings} 份`,
            reason: "依本次用餐人數調整",
          },
        ]
      : []),
    ...request.replacementRequests.map((item) => ({
      field: item.ingredientKey,
      before: item.ingredientKey,
      after: item.requestedReplacement,
      reason: "使用者要求替代",
    })),
  ];
  return {
