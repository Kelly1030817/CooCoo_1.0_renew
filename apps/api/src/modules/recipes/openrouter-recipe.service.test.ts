...
describe("recipe safety fallback",()=>{
  ...
  test("rejects incomplete AI dual guidance and falls back to reviewed content",async()=>{const previous=process.env.OPENROUTER_API_KEY;process.env.OPENROUTER_API_KEY="test";try{const recipe=structuredClone(brandSafeRecipes[0]);delete recipe.steps[0].compactInstruction;const result=await generateRecipe({style:"家常",excludeTitle:recipe.title,ingredientNames:[],restrictions:[],cookware:[{type:"電磁爐",capacity:null,limitations:[]}],budget:100,energyLevel:"normal",inventory:[]},{generate:async()=>({value:recipe,model:"test-model",costUsd:.01})});expect(result.source).toBe("brand_safe");expect(result.recipe.steps.every(step=>step.compactInstruction&&step.guidance?.successCue)).toBe(true)}finally{if(previous===undefined)delete process.env.OPENROUTER_API_KEY;else process.env.OPENROUTER_API_KEY=previous}})
  ...
  test("asks for one canonical recipe with aligned detailed and compact guidance",()=>{
    const prompt=buildRecipePrompt({style:"家常",ingredientNames:["雞蛋"],restrictions:[],cookware:[{type:"電磁爐",capacity:null,limitations:[]}],budget:null,energyLevel:"normal",inventory:[]});
