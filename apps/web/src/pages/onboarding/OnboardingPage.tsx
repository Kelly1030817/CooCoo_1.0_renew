...
export function OnboardingPage({ onComplete, onExit, canExit = false, initialStep }: { onComplete: () => void; onExit?: () => void; canExit?: boolean; initialStep?: number }) {
  ...
  return <main className="onboarding-shell">
    ...
    <section className="onboarding-card"><p className="eyebrow">STEP {step} / 5</p>
      ...
      {step === 2 && <><h1>你的餐桌與廚具</h1><label>通常幾人吃？<input name="household-servings" autoComplete="off" type="number" min="1" max="12" value={profile.householdServings} onChange={(e) => update({ householdServings: Number(e.target.value) })} /></label><fieldset><legend>可用廚具（至少一項）</legend><div className="choice-grid">{cookwareOptions.map((value) => <button type="button" className={profile.cookware.some((item) => item.type === value) ? "selected" : ""} onClick={() => toggleCookware(value)} key={value}>{value}</button>)}</div></fieldset><label>過敏或絕對不吃（逗號分隔）<input name="dietary-restrictions" autoComplete="off" placeholder="例如：花生、蝦…" onBlur={(e) => update({ restrictions: e.target.value.split(/[,，]/).map((value) => value.trim()).filter(Boolean).map((label, index) => ({ id: `restriction-${index}`, label, kind: "allergy" as const, ingredientKeys: [label], isHardLimit: true })) })} /></label></>}
