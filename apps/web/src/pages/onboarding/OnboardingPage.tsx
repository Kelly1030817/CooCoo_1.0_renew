...
export function OnboardingPage({ onComplete, onExit, canExit = false, initialStep }: { onComplete: () => void; onExit?: () => void; canExit?: boolean; initialStep?: number }) {
  ...
  const [mood, setMood] = useState<"listen" | "applause" | "care" | "sealed">("listen");
  ...
  const toggleBarrier = (value: HabitBarrier) => { setMood("listen"); update({ habitBarriers: profile.habitBarriers.includes(value) ? profile.habitBarriers.filter((item) => item !== value) : [...profile.habitBarriers, value] }); };
  const toggleCookware = (value: string) => { setMood("applause"); update({ cookware: profile.cookware.some((item) => item.type === value) ? profile.cookware.filter((item) => item.type !== value) : [...profile.cookware, { type: value, limitations: [] }] }); };
