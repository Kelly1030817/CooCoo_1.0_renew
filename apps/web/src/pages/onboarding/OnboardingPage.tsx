...
export function OnboardingPage({onComplete, onExit, canExit = false, initialStep}: {
  onComplete: () => void;
  onExit?: () => void;
  canExit?: boolean;
  initialStep?: number
  ...
  const toggleCookware = (value: string) => {
    cheer("applause");
    update({
      cookware: profile.cookware.some((item) => item.type === value) ? profile.cookware.filter((item) => item.type !== value) : [...profile.cookware, {
        type: value,
        limitations: []
      }]
    });
  ...
  return <main className="onboarding-shell">
    <section className="onboarding-device">
      ...
      <div className="onboarding-progress" role="progressbar" aria-label={`第 ${step} 步，共 3 步`} aria-valuemin={1} aria-valuemax={3} aria-valuenow={step}><span
        style={{width: `${step * 100 / 3}%`}}/></div>

      <div className="onboarding-content">
        <div className={`onboarding-stack onboarding-step-${step} step-slide-down`} key={step}>
          {step === 1 && <>
              <div className="chef-open"><span className="chef-open-icon material-symbols-outlined" aria-hidden="true">restaurant</span>
                  <div><strong>Hi！我是你的專屬主廚 CooCoo。</strong><p>沒有標準答案，我們只想讓第一次推薦更可行。你的回答會決定我怎麼挑菜、怎麼排餐。</p></div>
              </div>
              <StepCard title="平常可用時間">
                  <div className="slider-row"><input name="available-minutes" type="range" min="5" max="180" step="5" value={profile.availableMinutes}
                                                     onChange={(event) => update({availableMinutes: Number(event.target.value)})}/><strong>{profile.availableMinutes} 分鐘</strong>
                  </div>
              </StepCard>
              <StepCard title="常用餐期">
                  <div className="choices three">{([['breakfast', '早餐'], ['lunch', '午餐'], ['dinner', '晚餐']] as const).map(([value, label]) => <Choice key={value}
                                                                                                                                                            selected={profile.plannedMealSlots.includes(value)}
                                                                                                                                                            onClick={() => update({plannedMealSlots: profile.plannedMealSlots.includes(value) ? profile.plannedMealSlots.filter((item) => item !== value) : [...profile.plannedMealSlots, value]})}>{label}</Choice>)}</div>
              </StepCard>
              <StepCard title="喜歡的口味">
                  <div className="tag-input"><input value={flavorInput} placeholder="例如：清爽、台式、微辣" onChange={(event) => setFlavorInput(event.target.value)}/>
                      <button type="button" disabled={!flavorInput.trim()} onClick={addFlavor}>＋</button>
                  </div>
                  <div className="tag-list">{profile.preferredFlavors.map((flavor) => <span className="tag" key={flavor}>{flavor}
                    <button type="button" aria-label={`移除 ${flavor}`}
                            onClick={() => update({preferredFlavors: profile.preferredFlavors.filter((item) => item !== flavor)})}>×</button></span>)}</div>
              </StepCard>
              <StepCard title="過敏原或絕對不吃" description="可多選常見過敏原，其他項目也能自行新增；全部都會成為硬限制。">
                  <div className="restriction-chip-grid" aria-label="常見過敏原快選">{restrictionQuickOptions.map((option) => {
                    const selected = hasRestriction(profile.restrictions, option);
                    return <button type="button" className={`restriction-chip ${selected ? "selected" : ""}`} aria-pressed={selected} key={option.id}
                                   onClick={() => toggleHardRestriction(option)}><span>{option.label}</span><i aria-hidden="true">✓</i></button>;
                  })}</div>
                  <div className="tag-input restriction-input"><input name="dietary-restrictions" maxLength={24} placeholder="其他不能吃，例如：香菜"
                                                                      value={restrictionInput} onChange={(event) => setRestrictionInput(event.target.value)}
                                                                      onKeyDown={(event) => {
                                                                        if (event.key === "Enter") {
                      ...
                      <button type="button" disabled={!restrictionInput.trim()} aria-label="新增其他不能吃的食物" onClick={addRestriction}>＋</button>
                  </div>
                  <div
                      className="tag-list">{profile.restrictions.filter((item) => !restrictionQuickOptions.some((option) => option.id === item.id || option.label === item.label)).map((item) =>
                    <span className="tag" key={item.id}>{item.label}
                      <button type="button" aria-label={`移除 ${item.label}`}
                              onClick={() => update({restrictions: profile.restrictions.filter((restriction) => restriction.id !== item.id)})}>×</button></span>)}</div>
                  <p className="safety-note">硬限制會寫入 dietary_restrictions，所有推薦與 AI 調整都必須遵守。</p></StepCard>
          </>}

          {step === 2 && <>
              <div className="chef-open"><span className="chef-open-icon material-symbols-outlined" aria-hidden="true">skillet</span>
                  <div><strong>我先確認你真的能用什麼來煮。</strong><p>份量、廚具與日常卡點會直接影響我能推薦哪些料理。</p></div>
              </div>
              <StepCard className="step2-household" title="通常幾人吃？">
                  <div className="onboarding-counter"><span>人份</span>
                      <div>
                          <button type="button" onClick={() => update({householdServings: Math.max(1, profile.householdServings - 1)})}>−</button>
                          <strong>{profile.householdServings}</strong>
                          <button type="button" onClick={() => update({householdServings: Math.min(12, profile.householdServings + 1)})}>+</button>
                      </div>
                  </div>
              </StepCard>
              <StepCard className="step2-cookware" title="可用廚具" description="至少選一項；其他廚具也能自行新增。">
                  <div className="choices">{cookwareOptions.map((value) => <Choice key={value} selected={profile.cookware.some((item) => item.type === value)}
                                                                                   onClick={() => toggleCookware(value)}>{value}</Choice>)}</div>
                  <div className="tag-input"><input name="custom-cookware" maxLength={24} placeholder="例如：卡式爐、蒸烤箱" value={customCookware}
                                                    onChange={(event) => setCustomCookware(event.target.value)} onKeyDown={(event) => {
                    if (event.key === "Enter") {
                      event.preventDefault();
                      addCustomCookware();
                    }
                  }}/>
                      <button type="button" disabled={!customCookware.trim()} onClick={addCustomCookware}>＋</button>
                  </div>
                  <div className="tag-list">{profile.cookware.filter((item) => !cookwareOptions.includes(item.type)).map((item) => <span className="tag"
                                                                                                                                         key={item.type}>{item.type}
                    <button type="button" aria-label={`移除 ${item.type}`} onClick={() => toggleCookware(item.type)}>×</button></span>)}</div>
              </StepCard>
              <StepCard title="最常卡在哪裡？" description="可多選。這會決定我第一則提醒的方式。">
                  <div className="choices">{barriers.map(([value, label]) => <Choice key={value} selected={profile.habitBarriers.includes(value)}
                                                                                     onClick={() => toggleBarrier(value)}>{label}</Choice>)}</div>
              </StepCard>
          </>}

          {step === 3 && <>
              <div className="chef-open">
                  <span className="chef-open-icon material-symbols-outlined" aria-hidden="true">military_tech</span>
                  <div>
