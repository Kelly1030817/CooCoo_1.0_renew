...
export function TodayPage() {
      ...
      <ChefRevisitModal
        ...
        onAdjustTarget={async (newTarget) => {
          await api(
            "/weekly-goal",
            json("PATCH", {
              metric: data?.weeklyGoal.metric ?? "cooking_sessions",
              target: newTarget,
            }),
          );
        ...
        onRecordTakeout={async () =>
          "今晚休息也沒關係；本週目標可以隨生活調整，不會扣除 EXP 或留下失敗標記。"
        }
      />,
    );
  };
  ...
  useEffect(() => {
    ...
    api<TodayDecision>(
      ...
      .catch(
        (reason) =>
          active && setDecisionError(reason instanceof Error ? reason.message : "餐點載入失敗"),
      );
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    api<RecipeRecommendations>(
      ...
      .catch(
        (reason) =>
          active && setPurchaseError(reason instanceof Error ? reason.message : "補買推薦載入失敗"),
      )
      .finally(() => {
  ...
  const recommended =
    ticketMode === "purchase" ? activePurchaseItem?.recipe || recommendedFridge : recommendedFridge;
      ...
      .filter((item) =>
        targetRecipe.ingredients.some(
          (ingredient) =>
            ...
            (ingredient.ingredientKey === "greens" &&
              (item.name.includes("菜") || item.name.includes("蔬"))) ||
            (ingredient.ingredientKey === "noodles" &&
              (item.name.includes("麵") || item.name.includes("粉"))),
        ),
      )
      .map((item) => item.id);
    ui.open(
  ...
  let rankIndex = 0;
  for (let i = 0; i < CHEF_RANKS.length; i += 1)
    if (totalExp >= CHEF_RANKS[i].threshold) rankIndex = i;
  ...
  const alternativeMeals = recommended ? choices.filter((meal) => meal.id !== recommended.id) : [];
    ...
    <div className="today-page">
      ...
      <section className="today-hud" aria-label="主廚職階與 EXP">
        <div className="hud-row">
          <div className="hud-chef">
            ...
            <div className="hud-chef-text">
              ...
              <small>
                {nextBadge
                  ? `下一枚 · ${nextBadge.title} ${nextBadge.current}/${nextBadge.target}`
                  : nextRank
                    ? `距下一職階還有 ${nextRank.threshold - totalExp} EXP`
                    : "已達最高職階"}
              </small>
            </div>
          </div>
          <button type="button" className="hud-exp" onClick={openChefConsultation}>
            EXP {totalExp}
          </button>
        </div>
        ...
        <div className="hud-meta">
          <span>職階進度</span>
          <span>
            {totalExp} / {nextRank?.threshold ?? totalExp}
          </span>
        </div>
      </section>
              ...
              <div className="ticket-body">
                ...
                <p className="meal-subtitle">
                  {ticketMode !== "purchase"
                    ? decision?.notice ||
                      decisionError ||
                      "後台檢核您的可用廚具、時間與飲食限制，尚未找到同時符合全部條件的食譜。"
                    : purchaseError ||
                      purchaseResult?.notice ||
                      "目前沒有符合預算且補買不超過 2 項食材的候選食譜。"}
                </p>
                  ...
                  {recommended.ingredients
                    ...
                    .map((item) => {
                      const isMissing =
                        ticketMode === "purchase" &&
                        activeMissing.some(
                          (m) => m.ingredientKey === item.ingredientKey || m.name === item.name,
                        );
                      return (
                        <span
              ...
              {detailOpen && (
                <div className="ticket-detail" role="dialog" aria-label="餐點細節">
                  ...
                  <p className="detail-why">
                    {subtitles[recommended.title] || decision?.notice || "符合你的廚具與飲食設定。"}
                  </p>
                  {ticketMode === "purchase" && missingCount > 0 && (
                    <p className="detail-missing">
                      需補買{" "}
                      {activeMissing.map((m) => `${m.name} ${m.quantity} ${m.unit}`).join("、")}
                      {activePurchaseItem?.estimatedPurchaseCost
                        ? `（約 NT$ ${activePurchaseItem.estimatedPurchaseCost}）`
                        : ""}
                    </p>
                  )}
                  <p className="detail-note">
                    安全與過敏資訊仍以文字完整呈現；完成料理可獲得 +{EXP_POINTS.cooking_completed}{" "}
                    EXP，使用即期食材再加 +{EXP_POINTS.expiring_ingredient_used} EXP。
                  </p>
                </div>
              )}
            </article>
                    ...
                    {mission.done
                      ...
                      : mission.key === "cook_today"
                        ...
                        : mission.key === "eat_prepared"
                          ? "到冰箱加熱"
                          : `用即期食材煮${mission.hint ? ` · 庫存 ${mission.hint} 項即期` : ""}`}
                  </small>
                </span>
      ...
      <div className="weekstrip">
        ...
        <div className="wtxt">
          <b>
            本週 {weeklyGoal?.progress ?? 0} / {weeklyGoal?.target ?? 1} 餐
          </b>
          <div className="wbar">
            <span
              style={{
                width: `${Math.min(100, Math.round(((weeklyGoal?.progress ?? 0) / Math.max(1, weeklyGoal?.target ?? 1)) * 100))}%`,
              }}
            />
          </div>
        </div>
