...
describe("fridge inventory view", () => {
  test("groups the same ingredient, chamber and unit while preserving FIFO batches", () => {
    ...
    expect(groups[0]).toMatchObject({ qty: 750, daysLeft: 1, estimatedValue: 60, unpricedBatchCount: 1 });
  ...
  test("uses the backend confirmation windows for cold and longer-storage stock", () => {
    ...
    expect(needsInventoryConfirmation(item({ chamber: "cold", lastConfirmedAt: "2026-09-06T12:00:00.000Z" }), now)).toBe(true);
    expect(needsInventoryConfirmation(item({ chamber: "frozen", lastConfirmedAt: "2026-08-15T12:00:00.000Z" }), now)).toBe(false);
    expect(needsInventoryConfirmation(item({ chamber: "pantry", lastConfirmedAt: "invalid" }), now)).toBe(true);
  });
});
