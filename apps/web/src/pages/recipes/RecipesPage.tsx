...
function FreeformComposePanel(){
  ...
  return <section className="compose-panel"><header><div><p className="eyebrow">備料盤</p><h3>用冰箱裡的食材自由搭配</h3></div></header><div className="compose-section"><strong>料理風格</strong><div className="recipe-chips">{composeStyles.map((item)=><button type="button" key={item} className={style===item?"active":""} onClick={()=>setStyle(item)}>{item}</button>)}</div><small>使用「我的」已設定的廚具：{data.cookware.map((item)=>item.name||item.type).join("、")||"尚未設定"}</small></div><div className="compose-section"><div className="compose-section-header"><strong>已選食材</strong><span className="compose-badge">{selected.length} 項已選</span></div>{selected.length?<div className="compose-ingredients">{selected.map((item)=><button type="button" key={item.id} onClick={()=>removeFromTray(item.id)}><IngredientIcon name={item.name} size={18}/><span>{item.name} {item.qty}{item.unit}</span><small>移除</small></button>)}</div>:<p>從下方挑一項食材開始，不需要先決定完整菜色。</p>}</div><div className="compose-section"><strong>冰箱可用食材</strong><div className="compose-ingredients">{available.map((item)=><button type="button" key={item.id} onClick={()=>addToTray(item.id)}><IngredientIcon name={item.name} size={18}/><span>{item.name} {item.qty}{item.unit}</span><small>{item.daysLeft<=3?`即期 ${item.daysLeft} 天`:"加入"}</small></button>)}</div></div><button type="button" className="primary-btn w-full" disabled={!selected.length} onClick={start}>依備料盤產生料理建議</button></section>;
}
