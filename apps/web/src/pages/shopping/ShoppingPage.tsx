...
export function ShoppingPage(){
  ...
  const applyReplacement=async(shortageId:string,replacementName:string,qty:number,unit:string)=>{
    if(!activeTask)return;
    const payload={mealTaskId:activeTask.id,shortageRevision:activeTask.revision,shortageId,mode:"replace" as const,name:replacementName,quantity:qty,unit};
    await api<MealTask>(`/meal-tasks/${activeTask.id}/shortages/${shortageId}`,json("PATCH",payload));
    await refresh();ui.toast(`已採納主廚建議！食材已替換為「${replacementName}」`);
  };
  ...
  return <div className="shopping-mobile">
    ...
    <div className="shopping-responsive-layout">
      <div className="shopping-primary-column">
        ...
        {activeTask&&<section className={`meal-task-ticket ${activeTask.status==="ready"?"sealed":""}`}>
          ...
          <div className="ticket-body"><p className="ticket-label">MealTask 必買</p><h2>{activeTask.recipe.title}</h2><p className="ticket-copy">{`依已確認 ${activeTask.plannedTotalServings} 份計算 · 缺口 ${taskTotal} 項 · revision ${activeTask.revision}`}</p>
            ...
            <p className="ticket-note">{activeTask.status==="ready"?"食材已備妥。完成採買不發 EXP；料理完成後才會記錄成就。":`${remaining.length} 項仍缺。全部確認入庫後才會轉為 ready；部分買到會保留缺口。`}</p></div>
        </section>}
        ...
        <section className="shopping-coverage">
          ...
          <div><h2>{activeTask?(activeTask.status==='needs_replan'?'這道餐點待重新選擇':remaining.length?`還差 ${remaining.length} 樣就能煮`:'食材已備妥'):"先選一道料理開始"}</h2><p>{activeTask?`只計算已確認的 ${activeTask.plannedTotalServings} 份需求；採買入庫後自動蓋章。`:"確認料理後，必要食材會自動成為任務票上的章格。"}</p>{activeTask&&<div className="need-list">{remaining.slice(0,3).map(item=><span key={item.id}><IngredientIcon name={item.name} size={13}/>{item.name}</span>)}</div>}</div>
        </section>
