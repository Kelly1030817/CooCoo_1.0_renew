...
describe("mock repository v2",()=>{
  ...
  test("applies a completed three-step profile to the weekly goal",()=>expect(migrateMockState(createSeedState(),profile)).toMatchObject({weeklyGoal:{metric:"cooking_sessions",target:3},onboardingProfile:profile}));
});
