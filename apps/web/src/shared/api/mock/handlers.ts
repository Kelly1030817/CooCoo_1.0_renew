...
} from "../../../../../api/src/modules/meal-plans/meal-planning";
...
const mockStarterPrices: IngredientPrice[] = [
  {
    ...
    price: 90,
    source: "mock",
    observedAt: "2026-09-08T09:35:00+08:00",
  },
  {
    id: "c2",
    ingredientKey: "烏龍麵",
...
const planningContext = (
  weekStart: string,
  energyLevel: "low" | "normal" = "normal",
): MealPlanningContext => {
  const state = service.state();
  const profile = state.onboardingProfile;
  const mealSlots: MealSlot[] = profile?.plannedMealSlots?.length
    ? profile.plannedMealSlots
    : ["dinner"];
  const cookwareTypes =
    profile?.cookware.map((item) => item.type) ??
    state.cookware.flatMap((item) => [item.type, item.name]);
  return {
    weekStart,
...
function recommendationRequest(value: unknown): {
  ...
  const allowRepeat =
    "allowRepeat" in value && typeof value.allowRepeat === "boolean"
      ? value.allowRepeat
      : undefined;
  const energy =
    "energy" in value && (value.energy === "low" || value.energy === "normal")
      ? value.energy
      : undefined;
...
export const handlers = [
  ...
  http.post("/api/v1/auth/login", async ({ request }) => {
    try {
      return ok(
        service.login(validated(ContractSchemas.LoginRequestSchema, await request.json()).email),
      );
    } catch (e) {
      return error(e);
    }
  ...
  http.put("/api/v1/onboarding", async ({ request }) => {
    try {
      const profile = validated(ContractSchemas.OnboardingProfileSchema, await request.json());
    ...
    try {
      return ok(
        service.addInventory(
          validated(ContractSchemas.InventoryCreateSchema, await request.json()),
        ),
        201,
      );
    } catch (e) {
  ...
  http.post("/api/v1/inventory/:id/rescue", async ({ params, request }) => {
    try {
      const body = validated(ContractSchemas.RescueCommandSchema, await request.json());
  ...
  http.post("/api/v1/recipes/generate", async ({ request }) => {
    try {
      const b = validated(ContractSchemas.RecipeGenerateSchema, await request.json());
      const state = service.state();
      if (b.ingredientIds.some((id) => !state.inventory.some((item) => item.id === id)))
        throw new Error("ITEM_NOT_FOUND");
      const today = new Date(Date.now() + 8 * 3_600_000).toISOString().slice(0, 10);
      const decision = createTodayDecision(planningContext(weekOf(today)), {
        date: today,
        slot: "dinner",
      });
      const recipe = [decision.primary, ...decision.alternatives].find(
        (item) => item && item.title !== b.excludeTitle,
      );
      if (!recipe) throw new Error("NO_SAFE_RECIPE_AVAILABLE");
      return ok({
        recipe: await planningRepository.savePackage("preview", recipe),
        source: "brand_safe",
        notice: "本機預覽使用人工檢查過的安全食譜；連接正式 API 後才會呼叫 AI。",
      });
    } catch (e) {
      return error(e);
    }
  }),
  http.post("/api/v1/recipes/search", async ({ request }) => {
    try {
      const body = validated(ContractSchemas.RecipeSearchRequestSchema, await request.json());
      const state = service.state();
      return ok({
        items: searchRecipes(
          brandSafeRecipes,
          ...
          new Set(state.recipeFavoriteIds ?? []),
        ),
  ...
  http.put("/api/v1/recipes/:id/favorite", ({ params }) => {
    const state = service.state();
    const id = String(params.id);
    state.recipeFavoriteIds = Array.from(new Set([...(state.recipeFavoriteIds ?? []), id]));
    new BrowserStateRepository().write(state);
    return ok({ recipeId: id, favorite: true });
  }),
  http.delete("/api/v1/recipes/:id/favorite", ({ params }) => {
    const state = service.state();
    const id = String(params.id);
    state.recipeFavoriteIds = (state.recipeFavoriteIds ?? []).filter((value) => value !== id);
    new BrowserStateRepository().write(state);
    return ok({ recipeId: id, favorite: false });
  }),
  http.post("/api/v1/recipes/:id/adjustments/preview", async ({ params, request }) => {
    try {
      const body = validated(ContractSchemas.RecipeAdjustmentRequestSchema, await request.json());
      const recipe = brandSafeRecipes.find(
        (item) => item.id === params.id || item.recipeId === params.id,
      );
      if (!recipe) throw new Error("RECIPE_NOT_FOUND");
      const factor = body.servings / Math.max(1, recipe.servings);
      const adjustedRecipe = {
        ...structuredClone(recipe),
        id: `${recipe.id}:adjusted:${body.operationId}`,
        ...
        changes:
          recipe.servings === body.servings
            ...
            : [
                {
                  field: "servings",
                  before: `${recipe.servings} 份`,
                  after: `${body.servings} 份`,
                  reason: "依本次用餐人數調整",
                },
              ],
        missing: [],
        safetyChecks: ["飲食硬限制已重新檢查", "廚具維持原食譜需求", "確認後才建立 MealTask"],
  ...
  http.post("/api/v1/meal-tasks", async ({ request }) => {
    try {
      const body = validated(ContractSchemas.MealTaskCreateSchema, await request.json());
      const state = service.state();
      if (state.mealTasks?.some((task) => task.operationId === body.operationId))
        return ok(state.mealTasks.find((task) => task.operationId === body.operationId));
      const baseRecipe = brandSafeRecipes.find(
        (item) => item.id === body.recipePackageId || item.recipeId === body.recipePackageId,
      );
      if (!baseRecipe) throw new Error("RECIPE_NOT_FOUND");
      const preview = body.adjustmentPreviewId
        ? (state.recipeAdjustmentPreviews ?? []).find(
            (item) =>
        ...
        : undefined;
      if (body.adjustmentPreviewId && !preview) throw new Error("ADJUSTMENT_PREVIEW_INVALID");
      const task = createMealTask(
        body,
        preview?.adjustedRecipe ?? baseRecipe,
        state.inventory,
        state.onboardingProfile?.restrictions ?? [],
      );
      state.mealTasks = [...(state.mealTasks ?? []), task];
      new BrowserStateRepository().write(state);
      return ok(task, 201);
    } catch (e) {
      return error(e);
  ...
  http.post("/api/v1/meal-servings/:id/eat", async ({ params, request }) => {
    try {
      const body = validated(ContractSchemas.PreparedServingEatSchema, await request.json());
      return ok(service.eatPreparedServing(String(params.id), body.operationId));
    } catch (e) {
      return error(e);
    }
  }),
  ...
  http.post("/api/v1/meal-tasks/replan", async ({ request }) => {
    try {
      const body = validated(ContractSchemas.ShoppingResolutionCommandSchema, await request.json());
      const task = service.resolveShortage(body);
      if (!task) throw new Error("MEAL_TASK_NOT_FOUND");
      return ok(task);
    } catch (e) {
      return error(e);
  ...
  http.get("/api/v1/chef-chat/sessions", () =>
    ok((service.state().chefChatSessions ?? []).slice(-10).reverse()),
  ),
  http.post("/api/v1/chef-chat/sessions", async ({ request }) => {
    try {
      const body = validated(ContractSchemas.ChefChatSendSchema, await request.json());
      const state = service.state();
      const today = new Date().toISOString().slice(0, 10);
      const used = (state.chefChatSessions ?? [])
        .flatMap((session) => session.messages)
        .filter((message) => message.role === "user" && message.createdAt.startsWith(today)).length;
      if (used >= 30) throw new Error("AI_DAILY_LIMITED");
      ...
      const session = {
        ...
        messages: [
          { id: crypto.randomUUID(), role: "user" as const, content: body.message, createdAt: now },
          {
            ...
            content:
              "AI 目前未連線，我先用規則型協助：從即期食材選一項，再挑 30 分鐘內、符合廚具與飲食限制的食譜。你也可以到食譜頁用多食材搜尋。",
            createdAt: now,
          },
        ],
      };
  ...
  http.delete("/api/v1/chef-chat/sessions/:id", ({ params }) => {
    const state = service.state();
  ...
  http.get("/api/v1/meal-plans", async ({ request }) => {
    try {
      const weekStart =
        ...
        weekOf(new Date(Date.now() + 8 * 3_600_000).toISOString().slice(0, 10));
      const saved = await planningRepository.current("preview", weekStart);
      return ok(
        saved
          ? { ...saved, ...refreshAvailability(saved.plan, planningContext(weekStart).inventory) }
          : null,
      );
    } catch (e) {
      return error(e);
  ...
  http.post("/api/v1/meal-plans", async ({ request }) => {
    try {
      ...
      const saved =
        (await planningRepository.current("preview", body.weekStart)) ||
        (await planningRepository.save("preview", createMealPlan(context)));
  ...
  http.patch("/api/v1/meal-plans/meals/:id", async ({ params, request }) => {
    try {
      ...
      await planningRepository.reschedule(
        "preview",
        saved.plan,
        changed.meals.find((meal) => meal.id === params.id)!,
  ...
  http.post("/api/v1/cooking/outcomes", async ({ request }) => {
    try {
      const result = service.completeCooking(
        validated(ContractSchemas.CookingOutcomeCommandSchema, await request.json()),
      );
  ...
  http.post("/api/v1/shopping-items", async ({ request }) => {
    try {
      return ok(
        service.saveShopping(validated(ContractSchemas.ShoppingWriteSchema, await request.json())),
        201,
      );
    } catch (e) {
      return error(e);
  ...
  http.patch("/api/v1/shopping-items/:id", async ({ params, request }) => {
    try {
      const b = validated(ContractSchemas.ShoppingWriteSchema, await request.json());
    ...
    try {
      ...
      if (e instanceof Error && e.message === "MEAL_TASK_REVISION_CONFLICT") {
        return new Response(
          JSON.stringify({
            error: {
              code: "MEAL_TASK_REVISION_CONFLICT",
              message: "任務已更新，請刷新後重試",
              requestId: crypto.randomUUID(),
            },
          }),
  ...
  http.post("/api/v1/shopping/parse", async ({ request }) => {
    try {
      return ok(
        parseShoppingText(
          validated(ContractSchemas.ShoppingParseSchema, await request.json()).text,
        ),
      );
    } catch (e) {
      return error(e);
  ...
  http.post("/api/v1/shopping/analyze", () =>
    ok({
      ...
      recommendations: service
        .state()
        .shoppingItems.slice(0, 3)
        .map((item) => ({
          item,
          action: "buy_now" as const,
          reason: "本機預覽依採買清單與品項分類排序。",
        })),
      estimatedTotal: service
        .state()
        .shoppingItems.slice(0, 3)
        .reduce((total, item) => total + item.estCost, 0),
    ...
    try {
      return ok(
        service.updateSettings({
          cookware: validated(ContractSchemas.CookwareListSchema, await request.json()),
        }).cookware,
      );
    } catch (e) {
      return error(e);
  ...
  http.put("/api/v1/settings/recipes", async ({ request }) => {
    try {
      ...
      mockRecipeSettings = {
        purchaseBudget: incoming.purchaseBudget,
        confirmed: true,
        version: (mockRecipeSettings.version || 1) + 1,
      };
  ...
  http.post("/api/v1/recipes/recommendations", async ({ request }) => {
    try {
      ...
      const result = recommend(
        ...
        {
          mode: body.mode,
          purchaseBudget: body.purchaseBudget ?? 100,
          allowRepeat: body.allowRepeat ?? false,
          energy: body.energy,
        },
        mockStarterPrices,
        [],
