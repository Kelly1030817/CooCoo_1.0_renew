...
export function AddShoppingModal({onClose,item}:{onClose:()=>void;item?:ShoppingItem}){const ui=useContext(UiContext);const query=useQueryClient();return <Modal label="新增待採買食材" onClose={onClose}><ModalHeader title={item?'編輯待採買食材':'新增待採買食材'} onClose={onClose}/><form onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const value={shortageId:item?.shortageId,source:item?.source,name:formFieldString(f,'name'),category:formFieldString(f,'category'),qty:Number(f.get('qty')),unit:formFieldString(f,'unit'),estCost:Number(f.get('estCost')),checked:item?.checked||false,status:item?.status||'手動新增'};await api(item?`/shopping-items/${item.id}`:'/shopping-items',json(item?'PATCH':'POST',value));await query.invalidateQueries({queryKey:stateQueryKey});onClose();ui.toast(item?'採買項目已更新':'食材已加入採買清單')}} className="grid grid-cols-2 gap-md"><label className="col-span-2 text-xs font-bold text-on-surface-variant">食材名稱<input className="field" name="name" defaultValue={item?.name} readOnly={!!item?.shortageId} required/></label><label className="text-xs font-bold text-on-surface-variant">分類<select className="field" name="category" defaultValue={item?.category||'produce'}><option value="produce">新鮮蔬果</option><option value="protein">蛋白質與乳製品</option><option value="pantry">常備／調味</option><option value="other">其他</option></select></label><label className="text-xs font-bold text-on-surface-variant">數量<input className="field" type="number" name="qty" defaultValue={item?.qty||1}/></label><label className="text-xs font-bold text-on-surface-variant">單位<input className="field" name="unit" defaultValue={item?.unit||'包'} readOnly={!!item?.shortageId}/></label><label className="text-xs font-bold text-on-surface-variant">預估金額<input className="field" type="number" name="estCost" defaultValue={item?.estCost??0}/></label><button className="primary-btn col-span-2 mt-md">確認{item?'更新':'加入'}</button></form></Modal>}
