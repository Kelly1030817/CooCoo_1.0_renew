...
function CookingMode({ recipePackage, ingredientIds, onClose, onComplete }: { recipePackage: RecipePackage; ingredientIds: string[]; onClose: () => void; onComplete?: () => void }) {
  ...
  return <div className="cooking-mode" role="dialog" aria-modal="true" aria-label={`${recipePackage.title}料理模式`}><header><button onClick={onClose} aria-label="離開料理">×</button><div><small>{recipePackage.title}</small><strong>步驟 {stepIndex + 1} / {recipePackage.steps.length}</strong></div><button onClick={listen} aria-label="開始語音控制">🎙</button></header><div className="cooking-progress"><i style={{ width: `${((stepIndex + 1) / recipePackage.steps.length) * 100}%` }} /></div><main><span>STEP {String(step.order).padStart(2, "0")}</span><h2>{step.instruction}</h2>{step.safetyNote && <p className="safety-note">注意：{step.safetyNote}</p>}<button className="repeat-step" onClick={speak}>↻ 重複唸一次</button>{secondsLeft !== null && <div className="timer-display"><strong>{Math.floor(secondsLeft / 60)}:{String(secondsLeft % 60).padStart(2, "0")}</strong><button onClick={() => setTimerRunning((value) => !value)}>{timerRunning ? "暫停" : "繼續"}</button></div>}<p className="voice-status" aria-live="polite">{status}</p></main><footer><button onClick={previous} disabled={stepIndex === 0}>← 上一步</button>{stepIndex < recipePackage.steps.length - 1 ? <button className="next-step" onClick={next}>下一步 →</button> : <button className="next-step" onClick={() => setFinishing(true)}>完成料理 ✓</button>}</footer></div>;
}
