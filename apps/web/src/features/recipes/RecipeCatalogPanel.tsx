...
export function RecipeCatalogPanel(){
  ...
  const choose=(item:RecipeRecommendation)=>{
    ...
    ui.open(<PurchaseReminder item={item} budget={budget} allowRepeat={allowRepeat} onClose={ui.close} onUseInventory={()=>{ui.close();changeMode('inventory_only');}} onAdded={async()=>{await query.invalidateQueries({queryKey:stateQueryKey});ui.close();ui.toast('已加入購物清單；完成採買入庫後再開始料理。');setResult(null);}}/>);
  };
