...
describe("CooCoo growth", () => {
  ...
  test("does not award the same EXP event twice", () => {
    const first = awardExp([], {
      operationId: "cook-1",
      type: "cooking_completed",
      sourceId: "session-1",
    });
    const replay = awardExp(first.events, {
      operationId: "cook-1",
      type: "cooking_completed",
      sourceId: "session-1",
    });
  ...
  test("awards all twelve badges at the confirmed thresholds", () => {
    ...
    expect(
      awardBadges(result.awards, { cooking: 30, rhythm: 12, wasteLess: 15, exploration: 25 }).added,
    ).toHaveLength(0);
  });
  ...
  test("derives total EXP and the next badge", () => {
    ...
    expect(
      deriveGrowthProfile(result.events, [], {
        cooking: 1,
        rhythm: 0,
        wasteLess: 0,
        exploration: 1,
      ...
      totalExp: 30,
      rank: { name: "初火學徒" },
      nextBadge: { badgeKey: "cooking-1", current: 1, target: 1 },
    });
  });
});
