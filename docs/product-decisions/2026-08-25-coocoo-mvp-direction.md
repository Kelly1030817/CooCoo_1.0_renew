...
正式決策是：**以 `main` 的技術骨架作為核心，選擇性移植 `legacy-720-development` 的品牌視覺、行銷內容、陪伴感與遊戲化呈現。**
...
此策略已獲使用者確認並在 `codex/coocoo-integrated-mvp` 實作。兩個原始分支仍須保留，任何後續搬移都應維持共用契約、核心規則與頁面隔離。

## 7. 後續決策狀態

1. **已確認**：採用「`main` 技術骨架 + `legacy-720-development` 精選視覺與內容」的單一整合版。
2. **已確認**：省下金額使用本人外食比較價減去實際食材成本，最低為 0；使用者確認後才圓夢入帳。
3. **已確認**：十步必經 Onboarding 包含 Google／Email 登入，未完成草稿先留本機，登入後同步。
4. **已確認（2026-09-08 更新）**：後端透過 OpenRouter 解析有明細的發票圖片，保留逐欄信心與原圖，使用者確認後才入庫。
5. **部分完成、待真機驗證**：開始料理前保存核心文字、圖片索引、語音文字與計時；Web Speech 不保證離線可用。
6. **第一版保留**：主廚相談室、圓夢通行證、蓋章與完成慶祝；其他大型遊戲化功能延後。
