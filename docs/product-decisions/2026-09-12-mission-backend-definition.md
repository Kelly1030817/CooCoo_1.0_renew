...
- 日期：2026-09-12
- 狀態：**已定案並完成選項 A 實作**（四項 `待確認` 已於 2026-09-12 決議，見第 6 節）。未新增資料表、未修改 migration；`/state`、共用契約與 Today 已於 2026-09-13 對齊。
...
- `/state` 由後端依既有 `exp_events` 與庫存推導固定三項唯讀任務，並回傳 `key`、`label`、`done`、`reward`、`source` 與選用的 `hint`。
- `source` 對應既有 EXP 事件型別，任務完成不產生第二條寫入或發點路徑。
- Today 只讀 `/state.missions`；週節奏留在獨立進度列，「我的」顯示週餐次明細。
- 已增加台北跨午夜、固定三任務、今日餐次與餐單／完成紀錄合併測試。
