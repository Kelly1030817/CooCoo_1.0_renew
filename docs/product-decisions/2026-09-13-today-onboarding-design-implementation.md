...
1. 口味與飲食安全：時間滑桿、餐期、可移除口味標籤、過敏與禁食硬限制。
2. 餐桌與廚具：份量、預設／自訂廚具、多選卡點。
3. 登入與通行證：Google Auth、帳號同步、主廚檔案、親簽與蓋章；不建立、清空或修改冰箱庫存。

不再詢問熟練度、目前每週料理次數、料理預設指引、週主指標、可見週目標或提醒設定。完成時保留後端必要預設值，避免移除畫面後產生不完整資料。

蓋章是確認動作；完成資料由既有 `/onboarding` 寫入 Supabase。Google Auth、OCR 與硬限制規則不可被視覺層繞過。

料理模式預設採「詳細陪做」，使用者可在料理中切換為精簡步驟。後端維護一份權威料理包；兩種模式共用食材、份數、步驟順序、計時、完成標準與食安提醒。低體力則改變推薦菜色的時間、步驟數與鍋具條件，兩者不得混用。

## 驗收條件

- `bun run verify` 通過。
- 390px 手機 viewport 可完成三步且無水平溢位。
- 蓋章後進入 Today。
